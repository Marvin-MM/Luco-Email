...
//       // We are only returning the job, not creating new functions
//       // so the other functions are removed as they are not used.
...
class EmailQueueService {
  ...
  async initialize() {
    ...
    try {
      // First ensure Redis connection is established
      if (!redisManager.isConnected) {
        logger.info('Redis not connected, attempting to connect...');
        await redisManager.connect();
      }

      // Then initialize the queue service
      await queueService.initialize();
    ...
    try {
      ...
      try {
        await prisma.campaign.update({
          ...
          data: {
            status: 'FAILED',
            failureReason: `Queue initialization failed: ${error.message}`,
            completedAt: new Date(),
          },
        });
      } catch (updateError) {
        logger.error('Failed to update campaign status after queue error:', updateError);
      }
  ...
  /**
   * Check if the service is ready to process campaigns
   */
  isReady() {
    return this.isInitialized && !this.initializationError && redisManager.isConnected;
  }
