...
class QueueService {
  constructor() {
    ...
    this.redisConfig = {
      redis: {
        port: 6379,
        host: '127.0.0.1',
        // Add more configuration options for better reliability
        maxRetriesPerRequest: 3,
        retryDelayOnFailover: 100,
        enableReadyCheck: false,
        maxLoadingTimeout: 1000,
        // Parse Redis URL if provided
        ...(config.queue.redisUrl ? this.parseRedisUrl(config.queue.redisUrl) : {})
      },
      settings: {
  ...
  async initialize() {
    try {
      ...
      const testResult = await redisManager.healthCheck();
      if (testResult.status !== 'connected') {
        throw new Error(`Redis health check failed: ${testResult.error || 'Unknown error'}`);
      }

      logger.info('Redis connection verified, initializing queues...');
  ...
  async processCampaign(job) {
    ...
    try {
      ...
      logger.info(`Found ${totalRecipients} recipients to process`);
      ...
      for (let i = 0; i < recipients.length; i += batchSize) {
        ...
        logger.info(`Queued batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(totalRecipients/batchSize)}`, {
          campaignId,
          processed,
          totalRecipients
        });
